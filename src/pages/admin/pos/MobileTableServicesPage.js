...
const MobileTableServicesPage = () => {
  ...
  useEffect(() => {
    ...
    if (mode === 'view' && tableId) {
      (async () => await fetchExistingOrder())();
    }
  }, []);
  ...
  const fetchExistingOrder = async () => {
    try {
      ...
      const clickMap = {};
      filteredItems.forEach(i => (clickMap[i._id] = true));
  ...
  const updateQuantity = (itemId, delta, actionType = 'decrement') => {
    ...
    if (
      ...
      // setShowCartModal(false); // Removed to keep cart modal open underneath
      setShowPinModal(true);
      return;
    }

    const updated = cart
      .map(item => item._id === itemId ? { ...item, quantity: newQty } : item)
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (mode === 'view') {
        await axios.patch(`${BASE_URL}/api/orders/${orderId}/modify`, { updatedItems: payload.items, reason: 'Modified via mobile' }, {
          headers: { Authorization: `Bearer ${token}` },
        });
        alert('Order updated');
      } else {
  ...
  <>
    ...
    >
      ...
      <div style={{ marginTop: isMobile ? '56px' : 0 }}>
  ...
  {categories.map((cat, index) => (
    ...
    >
      {cat}
        ...
        <div className="item-grid" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start' }}>
          {menuItems
            ...
            .map(item => {
              ...
              return (
                <div
                  key={item._id}
                  className={`item-card ${clickedItems[item._id] ? 'clicked' : ''}`}
          ...
          <div className="cart-modal" style={{ zIndex: 1000 }} onClick={() => setShowCartModal(false)}>
            ...
            >
              ...
              {cart.map(item => (
                <div key={item._id} className="d-flex justify-content-between align-items-center mb-2 p-2 border rounded">
                  ...
                  <div className="d-flex align-items-center gap-2">
                    <button className="btn btn-outline-secondary btn-sm" onClick={() => updateQuantity(item._id, -1, 'decrement')}>-</button>
                    <span>{item.quantity}</span>
                    <button className="btn btn-outline-secondary btn-sm" onClick={() => updateQuantity(item._id, 1)}>+</button>
                    <button className="btn btn-outline-danger btn-sm" onClick={() => removeItem(item._id, 'delete')}>
              ...
              <div className="d-flex justify-content-between mt-4">
                <button className="btn btn-danger w-50 me-2" onClick={() => setShowCartModal(false)}>Close</button>
                <button className="btn btn-success w-50" onClick={handlePlaceOrder}>Place Order</button>
              </div>
              {mode === 'view' && (
                <button className="btn btn-primary w-100 mt-3" onClick={handlePayNow}>Pay Now</button>
              )}
            </div>
          </div>
        )}
